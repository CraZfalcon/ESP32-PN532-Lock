...
void loop() {
  ...
  if (nfc.tagPresent()) {
    ...
    HttpClient http = HttpClient(secure_layer, api_key, int(api_port));
    String serverAdress = "/api/DoorAccess?AccessTag_ID=" + String(KeyWord)
                                            + "&Id_Door=" + String(Door)
                                           + "&Location=" + String(Location);
    http.get(serverAdress.c_str());
    if (http.responseBody() == "true") {
      digitalWrite(5, HIGH);
